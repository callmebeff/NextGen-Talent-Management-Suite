...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        add_rem_button = ctk.CTkButton(self, text='Add/Remove Employee', font=('CTkFont', 15), height=40, command=self.open_employee_management_screen)
        add_rem_button.grid(column=0, row=2, padx=20, pady=15, sticky="ew")

        # Button: Lookup Employee
        emp_lookup_button = ctk.CTkButton(self, text='Employee Lookup', font=('CTkFont', 15), height=40)
        emp_lookup_button.grid(column=0, row=3, padx=20, pady=15, sticky="ew")

        # Button: Update user clearance level
        clearance_button = ctk.CTkButton(self, text='Update Clearance', font=('CTkFont', 15), height=40)
        clearance_button.grid(column=0, row=4, padx=20, pady=15, sticky="ew")

        # Button: Overtime: Approve/Reject
        overtime_button = ctk.CTkButton(self, text='Manage Overtime Requests', font=('CTkFont', 15), height=40)
        overtime_button.grid(column=0, row=5, padx=20, pady=15, sticky="ew")

        # Button: PTO: Approve/Reject
        pto_button = ctk.CTkButton(self, text='Manage PTO Requests', font=('CTkFont', 15), height=40)
        pto_button.grid(column=0, row=6, padx=20, pady=15, sticky="ew")
